...
def load_cases(path: Path) -> list[PerftCase]:
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line in handle:
            ...
            depth = int(depth_text.split("=")[1])
            nodes = int(nodes_text.split("=")[1])
            cases.append(PerftCase(fen=fen, depth=depth, nodes=nodes))
    return cases
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run perft regression cases against a Proton Chess binary.")
