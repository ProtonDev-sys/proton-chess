...
`stockfish18_3000_60+0.6.json` is the Windows AMD64 certification protocol. It fixes the opponent binary, runner, python-chess version, opening suite, clock, pairing, engine options, seed, game count and pass condition used for the Stockfish 3000 claim.
...
The launcher checks the platform, Stockfish binary, runner, python-chess version and every legal unique opening before starting. The match runner then records the Proton binary hash, tool hash, host, clocks, moves and per-game results in the output report.
